  ...
  <div class="card-container group [perspective:1000px] w-16 sm:w-24 h-24 sm:h-36 shrink-0 z-10 mx-1">
    ...
    >
      ...
      <div class="card-face card-back absolute inset-0 backface-hidden bg-blue-900 border-2 border-white rounded-lg shadow w-full h-full">
        <div class="w-full h-full border-2 border-dashed border-blue-400 rounded-md" />
      </div>
      ...
      <div
        ...
        :class="colorClass"
      >
        <div class="text-xs sm:text-base font-bold leading-none">
          {{ props.value }}
        </div>
        <div class="text-xl sm:text-3xl text-center flex-grow flex items-center justify-center">
          {{ suitSymbol }}
        </div>
        <div class="text-xs sm:text-base font-bold leading-none rotate-180 self-end">
          {{ props.value }}
        </div>
      </div>
    </div>
